...
def send_resp(mesg_pack=[],argsize=0,args=[]):
	...
	logging.debug("statusSend back:%s",resp_mesg_pack)
...
def exe_cmd_movement_error(mesg_pack):
	...
	return
#-----------------------------------------------------------------------
#		Function Name: exe_cmd_winch_op
#	Description: Check which winch control command it is
#-----------------------------------------------------------------------
...
#	Function Name: Thread for transfer file(does not work now)
#-------------------------------------------------------------
...
#		#exe_print(mesg_pack)
#		result_list = []
#		profile_flag = db_select_lr(5)
#		if profile_flag == 0:
#			# Need check power here 
#			if check5V() == 0:
#				# SW5V is off
#				mesg_pack[4] = chr(0x06)
#				result_list.append(chr(0x00))
#				send_resp(mesg_pack[:5],1,result_list)
#				return
#			else:
...
#			if status == -1 or status == 0:
#				mesg_pack[4] = chr(0x06)
#				result_list.append(chr(0x02))
#				send_resp(mesg_pack[:5],1,result_list)
#				return 
#			elif status == 3 or status == 1:
#				mesg_pack[4] = chr(0x06)
#				result_list.append(chr(0x03))
#				send_resp(mesg_pack[:5],1,result_list)
#				return
#			else:
#				print "profile 1 exe"
...
#		return
#----------------------------------------------------------------------------
#		Function Name: exe_cmd_profilerwinch_flag
#	Description: Deal with 0xb8 commands
#	Args: 0x00-------------check winch state
#		  0x01-------------check position offset
#		  0x02-------------check next profile time
#		  0x03-------------check current position
#----------------------------------------------------------------------------
